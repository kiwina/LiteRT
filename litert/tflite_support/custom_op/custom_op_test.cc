...
#include "litert/tflite_support/custom_op/custom_op.h"
...
TEST(OptionsExperimentalTest, MaxUnpooling2D) {
  ...
  LITERT_ASSERT_OK(tflite_support::AddCustomOp(options, reg));

  std::string model_path = litert::testing::GetLiteRtPath(
      "tflite_support/custom_op/max_unpooling_2d.tflite");
...
TEST(OptionsExperimentalTest, MaxUnpooling2D_Valid) {
  ...
  auto model_path = litert::testing::GetLiteRtPath(
      "tflite_support/custom_op/max_unpooling_2d_valid.tflite");
